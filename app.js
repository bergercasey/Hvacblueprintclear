...
// ---- Inline LEGACY pdf.js loader ----
async function ensurePDFJS(){
  log('ensurePDFJS: start (inline legacy)');
  if (!(window.pdfjsLib && window.pdfjsLib.getDocument)){
    // try to find the inline core script (already executed by being in DOM)
    if (!(window.pdfjsLib && window.pdfjsLib.getDocument)){
      throw new Error('Inline pdf.js core not present.');
    }
  }
  // Create a Blob URL for the inline worker code
  const workerTag = document.getElementById('pdfjs-worker-inline');
  if (!workerTag){ throw new Error('Inline pdf.js worker script not found'); }
  if (!ensurePDFJS._workerURL){
    const blob = new Blob([workerTag.textContent || ''], { type: 'text/javascript' });
    ensurePDFJS._workerURL = URL.createObjectURL(blob);
  }
  window.pdfjsLib.GlobalWorkerOptions.workerSrc = ensurePDFJS._workerURL;
  log('pdf.js ready (inline legacy)');
